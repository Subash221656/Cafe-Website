...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 10, "COFFEE RUSH Report", 0, 1, 'C');
...
$pdf->ChapterBody("Total Revenue Today: $ " . number_format($totalRevenueToday, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Week: $ " . number_format($totalRevenueThisWeek, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Month: $ " . number_format($totalRevenueThisMonth, 2));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Year: $ " . number_format($totalRevenueThisYear, 2));
$pdf->Ln();
...
$header = array('Date','Week' , 'Revenue ($)');
$data = array();
...
$header = array('Date','Month' , 'Revenue ($)');
$data = array();
...
$header = array('Date','Day' , 'Item Category', 'Revenue ($)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
...
$header = array('Date','Week' , 'Item Category', 'Revenue ($)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
...
$header = array('Date','Month' , 'Item Category', 'Revenue ($)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
...
$header = array('Date', 'Item Category', 'Revenue ($)');
$data = array();
